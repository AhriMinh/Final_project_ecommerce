import React from 'react'
import { lightLogo, paymentLogo, shoppingLogo } from '../assets'
import { ImGithub } from "react-icons/im";
import { FaYoutube, FaFacebookF, FaTwitter, FaInstagram, FaHome } from "react-icons/fa";
import { BsPaypal, BsPersonFill } from "react-icons/bs";
import { MdLocationOn } from "react-icons/md";

const Footer = () => {
  return (
    <div className='bg-black text-[#949494] py-20 font-titleFont'>
        <div className='max-w-screen-xl mx-auto grid grid-cols-4'>
            <div className='flex flex-col gap-3'>
                <img className='w-48 ml-6' src={lightLogo} alt="" />
                <p className='text-white text-sm tracking-wide ml-12'>Bonono.com</p>
                <img className="w-64" src={paymentLogo} alt="payment-logo" />
                <div className='flex gap-5 text-lg text-gray-400 ml-10'>
                  <ImGithub className="hover:text-white duration-300 cursor-pointer" />
                  <FaYoutube className="hover:text-white duration-300 cursor-pointer" />
                  <FaFacebookF className="hover:text-white duration-300 cursor-pointer" />
                  <FaTwitter className="hover:text-white duration-300 cursor-pointer" />
                  <FaInstagram className="hover:text-white duration-300 cursor-pointer" />
                </div>
            </div>

            <div>
              <h2 className="text-2xl font-semibold text-white mb-4 mt-7">
                locate us
              </h2>
              <div className='text-base flex flex-col gap-2'>
                <p>No. 73, Lang Street</p>
                <p>Mobile: (+84) 903491250</p>
                <p>e-mail: bonono@gmail.com</p>
                <p>github: ShujinOfAhri</p>
              </div>
            </div>

            <div>
              <h2 className="text-2xl font-semibold text-white mb-4 mt-7">
                profile
              </h2>
              <div className='flex flex-col gap-2 text-base'>
                <p className='flex items-center gap-3 hover:text-white duration-300 cursor-pointer'> <span><BsPersonFill /> </span> {" "} My ccount </p>
                <p className='flex items-center gap-3 hover:text-white duration-300 cursor-pointer'> <span><BsPaypal /> </span> {" "} Check out </p>
                <p className='flex items-center gap-3 hover:text-white duration-300 cursor-pointer'> <span><FaHome /> </span> {" "} Order Tracking </p>
                <p className='flex items-center gap-3 hover:text-white duration-300 cursor-pointer'> <span><MdLocationOn /> </span> {" "} Help & Support </p>
              </div>
            </div>

            <div className='flex flex-col justify-center'>
              <input className='bg-transparent border px-4 py-2 text-sm' placeholder="e-mail" type="text"/>
              <button className='text-sm border text-white border-t-0 hover: bg-gray-900 active:bg-white active:text-black'>Subcribe</button>
            </div>




        </div>
    </div>
  )
}

export default Footer